...
export default function PreviewHomeworkDetails() {
  ...
  const { student_id, homework_id } = router.query;
  ...
  useEffect(() => {
    if (router.isReady) {
      if (!student_id || !homework_id) {
        if (student_id) {
          router.replace(`/dashboard/manage_online_system/preview_student_homeworks?student_id=${student_id}`);
        } else {
          router.replace('/dashboard/manage_online_system/preview_student_homeworks');
        }
        return;
      }
    }
  }, [router.isReady, student_id, homework_id, router]);

  useEffect(() => {
    if (!student_id || !homework_id) return;

    const fetchDetails = async () => {
      try {
        setIsLoading(true);
        
        const response = await apiClient.get(`/api/students/${student_id}/preview-homework-details?homework_id=${homework_id}`);
        if (response.data.success) {
          setHomework(response.data.homework);
          setResult(response.data.result);
        } else {
          router.push(`/dashboard/manage_online_system/preview_student_homeworks?student_id=${student_id}`);
        }
      } catch (err) {
        console.error('Error fetching details:', err);
        router.push(`/dashboard/manage_online_system/preview_student_homeworks`);
      } finally {
        setIsLoading(false);
      }
    };
