...
export default function PreviewQuizDetails() {
  ...
  const { student_id, quiz_id } = router.query;
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        const response = await apiClient.get(`/api/students/${student_id}/preview-quiz-details?quiz_id=${quiz_id}`);
        if (response.data.success) {
          setQuiz(response.data.quiz);
          setResult(response.data.result);
        } else {
          router.push(`/dashboard/manage_online_system/preview_student_quizzes?student_id=${student_id}`);
        }
      } catch (err) {
        console.error('Error fetching details:', err);
        router.push(`/dashboard/manage_online_system/preview_student_quizzes`);
      } finally {
        setIsLoading(false);
      }
    };
