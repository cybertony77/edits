  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        ...
        // Get mock exam and saved result
        const response = await apiClient.get(`/api/students/${profile.id}/mock-exam-details?mock_exam_id=${id}`);
        if (response.data.success) {
          setMockExam(response.data.mockExam);
          setResult(response.data.result);
        } else {
          // Mock exam not found or no result - redirect immediately
          router.replace('/student_dashboard/my_mock_exams');
          return;
        }
      } catch (err) {
        // Handle 404 or 403 silently (expected when mock exam doesn't exist)
    ...
    const fetchImages = async () => {
      ...
      for (const question of mockExam.questions) {
        ...
        for (const publicId of ids) {
          ...
          try {
            const response = await apiClient.get(`/api/online_mock_exams/image?public_id=${encodeURIComponent(publicId)}`);
    ...
    return (
      <div style={{ 
        ...
        <div style={{ maxWidth: 1000, margin: "40px auto", padding: "20px 5px 20px 5px" }}>
          <Title backText="Back" href="/student_dashboard/my_mock_exams">Mock Exam Details</Title>
  ...
  return (
    <div className="page-wrapper" style={{ 
      ...
      <div className="page-content" style={{ maxWidth: 800, margin: "40px auto", padding: "12px" }}>
        <Title backText="Back" href="/student_dashboard/my_mock_exams">
