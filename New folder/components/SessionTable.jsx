    ...
    <Table ref={tableRef} style={{ width: '100%', tableLayout: 'fixed' }}>
      ...
      <Table.Tbody>
        {data.length === 0 ? (
          <Table.Tr>
              <Table.Td 
              colSpan={1 + 1 + (showGrade ? 1 : 0) + (showSchool ? 1 : 0) + 1 + (showMessageState ? 1 : 0) + (showWhatsApp ? 1 : 0) + 1 + (showMessageState ? 1 : 0) + (showWhatsApp ? 1 : 0) + (showParentsPhone2 ? 1 : 0) + (showAddress ? 1 : 0) + (showMainCenter ? 1 : 0) + (showAccountStatus ? 1 : 0) + (showAvailableSessions ? 1 : 0) + (showStatsColumns ? 1 : 0) + (showHW ? 1 : 0) + (showQuiz ? 1 : 0) + (showComment ? 1 : 0) + (showComment ? 1 : 0) + 3} 
              style={{ 
                border: 'none', 
                padding: 0,
                textAlign: 'center',
